...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def calculate_attention(self, query, key, value, mask):
        ...
        # mask: (n_batck, 1, seq_len, seq_len)
        d_k = key.shape[-1]
        attention_score = torch.matmul(query, key.transpose(-2, -1)) # QK^T, (n_batch, h, seq_len, seq_len)
        attention_score = attention_score / math.sqrt(d_k) # QK^T/sqrt(d_k)
        if mask is not None: # masked self-attention
            attention_score = attention_score.masked_fill(mask==0, 1e-9)
        attention_prob = F.softmax(attention_score, dim=-1) # softmax(QK^T/sqrt(d_k)), (n_batch, h, seq_len, seq_len)
    ...
    def forward(self, query, key, value, mask=None):
        ...
        out = out.contiguous().view(n_batch -1, self.d_model) # concat, (n_batch, seq_len, d_model)
