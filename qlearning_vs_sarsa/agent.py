...
class AGENT:
    def __init__(self, env, is_upload=False, alg=''):
        ...
        if is_upload:
            ...
            if alg == 'qlearning':
                ...
                sarsa_results = np.load('./result/sarsa.npz')
                self.V_values = sarsa_results['V']
                self.Q_values = sarsa_results['Q']
                self.policy = sarsa_results['PI']
            else:
                print("[Error] Unvalid algorithm!")
                exit()
        else:
    ...
    def Q_learning(self, discount=1.0, alpha=0.01, max_seq_len=500,
        ...
        draw_value_image(iteration=1, value_image=np.round(self.V_values, decimals=2), env=self.env)
        self.policy_summary(epsilon)
        draw_policy_image(iteration=1, policy_image=np.round(self.policy, decimals=2), env=self.env)
    ...
    def SARSA(self, discount=1.0, alpha=0.01, max_seq_len=500,
                            epsilon=0.3, decay_period=20000, decay_rate=0.9):
        ...
        self.policy_summary(epsilon)
        draw_policy_image(iteration=1, policy_image=np.round(self.policy, decimals=2), env=self.env)
        np.savez('./result/sarsa.npz', Q=self.Q_values, V=self.V_values, PI=self.policy)
        return self.Q_values, self.V_values, self.policy
